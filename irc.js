...
function Client(server, nick, opt) {
    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "err_nomotd":
                ...
                break;
            case "rpl_namreply":
                var channel = self.chans[(message.args[2]).toLowerCase()];
    ...
    // Ping sender.
    self.addListener('registered', function() {
        var sendPing = function() {
            var cookie = '' + Math.random();
            var timeout = setTimeout(function() {
                if (self.opt.debug) {
                    util.log('No ping response in ' + self.opt.pingTimeout + ' seconds.');
                }
                self.conn.end();
            }, self.opt.pingTimeout * 1000);
            self.addListener('pong', function(pongCookie) {
                if (pongCookie == cookie) {
                    clearTimeout(timeout);
                    setTimeout(sendPing, pingRate * 1000);
                }
            });
        }
    });
...
Client.prototype.connect = function ( retryCount ) { // {{{
    ...
    var server = self.opt.server;
    if (typeof(server) != 'string') {
        if (!self.serverNum || self.serverNum > self.opt.server.length) {
            self.serverNum = 0;
        }
        server = server[self.serverNum];
        self.serverNum++;
    }
    ...
    if (self.opt.secure) {
        ...
        self.conn = tls.connect(self.opt.port, server, creds, function() {
        ...
        self.conn = net.createConnection(self.opt.port, server);
    }
